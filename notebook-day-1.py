...
app._unparsable_cell(
    ...
    $$ \sum \vec{F}_{\text{ext}} = M \vec{a}_{\text{COM}} $$
    où $M$ est la masse du booster et $\vec{a}_{\text{COM}} = (\ddot{x}, \ddot{y})$ est l'accélération de son centre de masse.

    Les forces externes agissant sur le booster sont la **gravité** $\vec{F}_g = (0, -Mg)$ et la **force du réacteur** $\vec{F}_{\text{réacteur}}$.
    Les composantes de la force du réacteur sont :
     f_x = -f \sin(\theta + \phi) 
    $$ f_y = f \cos(\theta + \phi) $$

    En appliquant le PFD :
    Sur l'axe X : $ M \ddot{x} = f_x \implies \ddot{x} = \frac{1}{M} \left( -f \sin(\theta + \phi) \right) $
    Sur l'axe Y : $ M \ddot{y} = f_y - Mg \implies \ddot{y} = \frac{1}{M} \left( f \cos(\theta + \phi) \right) - g $
...
@app.cell
def _(mo):
    mo.image(src="public/images/MOMENT_DINERTIE.png")
    return
...
def _(mo):
    ...
    Le moment d'inertie d'une tige mince de longueur $L_{\text{tige}}$ et de masse $M_{\text{tige}}$ par rapport à un axe perpendiculaire passant par son centre de masse est donné par :
    $ J_{\text{COM}} = \frac{1}{12} M_{\text{tige}} L_{\text{tige}}^2 $
    Dans notre cas, $M_{\text{tige}} = M$ et $L_{\text{tige}} = 2\ell$.
...
@app.cell
def _(mo):
    mo.image(src="public/images/titl.png")
    return
...
def _(mo):
    ...
    $$ \sum \tau_{\text{COM}} = J \ddot{\theta} $$
    où $\sum \tau_{\text{COM}}$ est la somme des moments (couples) des forces externes par rapport au centre de masse, $J$ est le moment d'inertie et $\ddot{\theta}$ est l'accélération angulaire.

    La **gravité** s'applique au centre de masse, son moment est nul.
    La **force du réacteur** $\vec{F}_{\text{réacteur}}$ est appliquée à la base du booster (position $-\ell$ le long de l'axe du booster par rapport au CoM).
    Le vecteur position de la base par rapport au CoM est $\vec{r}_{\text{base/COM}} = (\ell\sin\theta, -\ell\cos\theta)$.
    La force du réacteur est $\vec{F}_{\text{réacteur}} = (-f\sin(\theta+\phi), f\cos(\theta+\phi))$.
    Le moment de cette force est : $\tau_{\text{réacteur}} = (\vec{r}_{\text{base/COM}} \times \vec{F}_{\text{réacteur}})_z = -\ell f \sin\phi$.

    Ainsi, l'équation du mouvement de rotation est :
    $ J \ddot{\theta} = -\ell f \sin\phi \implies \ddot{\theta} = \frac{-\ell f \sin\phi}{J} $
    En substituant $J = \frac{1}{3} M \ell^2$ :
    $ \ddot{\theta} = \frac{-\ell f \sin\phi}{\frac{1}{3}M\ell^2} = \frac{-3f\sin\phi}{M\ell} $

    Pour la simulation, avec la vitesse angulaire $\omega = \dot{\theta}$ :
    1.  $ \dot{\theta} = \omega $
    2.  $ \dot{\omega} = \frac{-3f\sin\phi}{M\ell} $
    Ces équations sont implémentées dans `redstart_dynamics_ode` (voir section Simulation).
    """
    )
    return
